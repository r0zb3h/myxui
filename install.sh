...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/r0zb3h/myxui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        last_version=$1
        url="https://github.com/r0zb3h/myxui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
